...
exports.ServerInstallError = void 0;
...
exports.ServerInstallError = ServerInstallError;
const DEFAULT_DOWNLOAD_URL_TEMPLATE = 'https://github.com/blackboxeditor/blackbox-updates-server/releases/download/test/blackbox-server-${os}-${arch}.tar.gz';
async function installCodeServer(conn, serverDownloadUrlTemplate, extensionIds, envVariables, platform, useSocketPath, logger) {
    ...
    const vscodeServerConfig = await (0, serverConfig_1.getVSCodeServerConfig)();
    ...
    if (platform === 'windows') {
        ...
        commandOutput = await conn.execPartial(command, (stdout) => endRegex.test(stdout));
    }
    else {
...
function parseServerInstallOutput(str, scriptId) {
...
function generateBashInstallScript({ id, quality, version, commit, release, extensionIds, envVariables, useSocketPath, serverApplicationName, serverDataFolderName, serverDownloadUrlTemplate }) {
...
DISTRO_BLACKBOX_RELEASE="${release ?? ''}"
...
SERVER_DOWNLOAD_URL="$(echo "${serverDownloadUrlTemplate.replace(/\$\{/g, '\\${')}" | sed "s/\\\${quality}/$DISTRO_QUALITY/g" | sed "s/\\\${version}/$DISTRO_VERSION/g" | sed "s/\\\${commit}/$DISTRO_COMMIT/g" | sed "s/\\\${os}/$PLATFORM/g" | sed "s/\\\${arch}/$SERVER_ARCH/g" | sed "s/\\\${release}/$DISTRO_BLACKBOX_RELEASE/g")"
...
function generatePowerShellInstallScript({ id, quality, version, commit, release, extensionIds, envVariables, useSocketPath, serverApplicationName, serverDataFolderName, serverDownloadUrlTemplate }) {
    const extensions = extensionIds.map(id => '--install-extension ' + id).join(' ');
