...
var NotebookEditorSimpleWorker = class {
  ...
  async $computeDiffWithCellIds(original, modified) {
    ...
    const orderOrOriginalCellIds = originalCellIds.filter((id2) => modifiedCellIds.includes(id2)).join(",");
    const orderOrModifiedCellIds = modifiedCellIds.filter((id2) => originalCellIds.includes(id2)).join(",");
